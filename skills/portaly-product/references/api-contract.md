...
- `metadata`: free-form string keys, max 20 keys, each value <= 500 chars. Echoed back. Do not put secrets here. **Because it is echoed into the signed callback body, put your own data under the `tracking` key as a JSON string** — it is committed in the callback vectors and verifies under every adapter. Any other custom key is only verifiable by the Node/WebCrypto adapters; the Python/Go v1 adapters fail closed on it.
