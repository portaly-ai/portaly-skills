...
_SUPPORTED_KEY_ORDER = tuple(
    ...
    billingPeriod bindMode bundleId buyerEmail buyerName campaign
    ...
    carrierType cart_id chargedAt checkoutSessionId child code codeId company
    companyId completedAt currency currentPeriodStartedAt customerEmail
    customerName customerPhone cycles discount discountedAmount duration empty
    enabled error event failedAt failureCount failureReason finalAmount flags
    invoice lastChargedAt lastFailureAt lastFailureReason lastPaymentReference
    list merchantConsumerId merchantOrderNumber metadata mode negative nested
    ...
    paymentMethodId paymentProvider paymentReference planId planIds planName
    productId productName profileId recTradeId recurringReady refundedAmount
    refundedAt refundFailedAt refundFailureReason refundFailureRetryable
