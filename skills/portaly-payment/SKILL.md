...
- **Keep the two kinds of id apart.** Your own idempotency is the step 7 key — stable across redeliveries, one per occurrence. Meta's `event_id` deduplicates a server event against the *browser* event for the same purchase (within 48 hours of Meta receiving the first), which exists only for the initial checkout — so use `sessionId` there, read from the merchant's own order record rather than the query string. **Give each renewal charge its own `event_id`** (`paymentId` on `payment.succeeded`): Meta asks for a unique id per event instance, and `subscriptionId === sessionId` means reusing `sessionId` gives every charge the same value. Do not argue from the 48-hour number — that window is documented for browser-versus-server dedup only; Meta collapses redundant server events too but publishes no window for it.
