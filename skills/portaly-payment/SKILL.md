...
- Fire `purchase` on the merchant's **own success page**: same origin, so the `_ga` cookie and the session's campaign are intact and GA4 attributes it correctly with no extra work. No cross-domain linker is needed — precisely because the payment page runs no merchant tag. But it is the accurate path, not the complete one: after a live 91APP payment the buyer returns to a Portaly page and must **click** through to the merchant, so anyone who closes the tab never fires it. Always pair it with the callback.
...
- **Keep the two kinds of id apart.** Your own idempotency stays `event + sessionId` / `event + paymentId` **or `paymentReference`** / `event + orderId`, as in step 7. Meta's `event_id` deduplicates a server event against the *browser* event for the same purchase — which exists only for the initial checkout — so use `sessionId` there, read from the merchant's own order record rather than the query string. Never reuse `sessionId` as the `event_id` for renewals: `subscriptionId === sessionId`, so every renewal carries the same value and Meta would discard the second month onward.
- **Two field traps on renewal payloads.** `payment.failed` carries **no `paymentId`** (only `payment.succeeded` does, and even there it can be empty) — fall back to `paymentReference`, or every failed renewal across every subscriber collapses onto one key and dunning silently stops. And `merchantOrderNumber` *is* present but is the value frozen at checkout, so using it as a GA4 `transaction_id` makes GA4 dedup every renewal after the first — build a per-charge id instead.
- **Ad identifiers (`client_id`, `session_id`, `_fbp`, `_fbc`, `utm_*`) belong in the merchant's own store, not in `metadata`.** Capture them on first landing and at checkout, keyed by `merchantOrderNumber`, then join on the callback. None of those key names is on the committed signing whitelist, so putting one in `metadata` makes a Python or Go receiver 401 the whole callback and stops order reconciliation.
- Load `references/conversion-tracking.md` for the session-survival conditions with their Google sources, copy-ready snippets, the GA4 Measurement Protocol requirements, and which `metadata` keys actually are safe.
