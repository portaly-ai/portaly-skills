...
- **Required setup merchants never think of:** add `portaly.ai` to GA4's unwanted-referrals list (Admin → Data streams → Web → Configure tag settings → Show all → List unwanted referrals). Without it, a restarted session is attributed to `portaly.ai / referral` and the campaign is lost. Say this first, every time.
...
- **Keep the two kinds of id apart.** Your own idempotency stays `event + sessionId` / `event + paymentId` / `event + orderId` as in step 7. Meta's `event_id` only deduplicates a server event against a *browser* event, which exists solely for the initial checkout — use `sessionId` there. Never reuse `sessionId` as the `event_id` for renewals: `subscriptionId === sessionId`, so every renewal would carry the same value and Meta would discard the second month onward. Renewal payloads carry `paymentId` and no `sessionId` key at all.
- Load `references/conversion-tracking.md` for the session-survival conditions with their Google sources, copy-ready snippets, and the `metadata` limitation that blocks the Python/Go callback adapters from carrying `client_id`.
