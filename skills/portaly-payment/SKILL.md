...
- **Keep the two kinds of id apart.** Your own idempotency is the step 7 key — stable across redeliveries, one per occurrence. Meta's `event_id` deduplicates a server event against the *browser* event for the same purchase (within 48 hours of Meta receiving the first), which exists only for the initial checkout — so use `sessionId` there, read from the merchant's own order record rather than the query string. **Give each renewal charge its own `event_id`** (`paymentId` on `payment.succeeded`): `subscriptionId === sessionId`, so reusing `sessionId` gives every charge the same value, and a dunning retry lands a day later — inside Meta's 48-hour window — where it really would be discarded.
