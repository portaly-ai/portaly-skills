...
- **Key renewals on a per-attempt timestamp, not a payment id.** `payment.failed` carries no `paymentId`, and `paymentReference` is an empty string on effectively every 91APP failure — either choice collapses every failed renewal across every subscriber onto one key and dunning silently stops. Use `event + subscriptionId + failedAt` (and `chargedAt` for `payment.succeeded`); not `failureCount`, which resets on success.
...
- **Ad identifiers belong in the merchant's own store, not in `metadata`.** Capture `utm_*` / `gclid` / `fbclid` on first landing, read GA4's ids via `gtag('get', …)` rather than parsing the `_ga_*` cookie (Google does not document its format and changed it in 2025), and key the record on the **`sessionId`** returned by create-session — not `merchantOrderNumber`, which is optional and absent from `checkout.failed`. Join on the callback.
