...
1. **One-time, fixed-price plans only.** Subscription plans cannot have promotion. Neither can dynamic-pricing plans — a promoter has to see a fixed price to know what a referral is worth.
...
```
GET {PORTALY_API_HOST}/api/creator-subscription/plans
Authorization: Bearer {PORTALY_API_KEY}
```

Keep only plans with `status: "active"`, `billingPeriod: "one-time"` and `pricingType: "fixed"`. Then handle what's left:

- **Qualifying plans exist** → list name, amount and id, and ask which one(s) to switch on.
- **No plans at all** → offer to create a one-time fixed-price plan first (hand off to `portaly-payment`).
- **Only monthly / yearly** → say plainly: *"Buyer promotion only works on one-time payment plans, so your subscription plans can't use it. If there's a single course or one-off product you want promoted, create a one-time plan for it and we can switch promotion on for that."* **Do not offer a timeline, a roadmap, or "coming soon" for subscription support.**
- **One-time but `pricingType: "dynamic"`** → this is the common case for sites that compute the price themselves. Say: *"Promotion needs a fixed plan price so promoters can see what they earn per referral. Your plan takes its amount at checkout, so Portaly has no fixed number to show."* Then **offer the fix and do it with them**: create one fixed-price one-time plan per product, and change their checkout call to send the matching `planId` instead of computing an `amount`. This is a simplification, not a rewrite — and it also gives them per-product orders and stats on Portaly's side. Do not promise dynamic-pricing support.

### 2. Agree on the commission rate

Read the allowed range from the API rather than hardcoding it:

```
GET {PORTALY_API_HOST}/api/creator-subscription/plans/{planId}/promotion
Authorization: Bearer {PORTALY_API_KEY}

→ { "data": { "planId", "enabled", "commissionRate", "promotionUrl",
              "serviceFeeRate", "minCommissionRate",
              "maxCommissionRate", "defaultCommissionRate" } }
```

Then work it out in the creator's own numbers, because a percentage means nothing on its own:

> "This plan is NT$1,500. At {defaultCommissionRate}% a promoter earns about NT$225 per sale, and Portaly takes a {serviceFeeRate}% service fee. The allowed range is {minCommissionRate}–{maxCommissionRate}%. Courses and other high-margin, word-of-mouth products usually need the higher end to get anyone actually sharing. What do you want to use?"

If they pick something outside the range, say what the range is and ask again — do not silently clamp it.

### 3. Switch promotion on

```
PUT {PORTALY_API_HOST}/api/creator-subscription/plans/{planId}/promotion
Authorization: Bearer {PORTALY_API_KEY}
Content-Type: application/json

{ "enabled": true, "commissionRate": 20, "promotionUrl": "https://theirsite.example/products/course" }
```

- `promotionUrl` is **the page on the creator's own site that referral links open** — normally this plan's product page. Required the first time.
- **This decides who gets paid what. With a live key, restate the plan name, the rate and the mode, and wait for an explicit yes before sending.**
- Read it back with `GET` and tell the creator the rate that actually took effect.
- On failure, stop — do not write any attribution code. A referral link handed out while the switch is off earns the promoter nothing. See `references/promotion-api.md` for the error codes; the two worth knowing here are `PROMOTION_LOCALE_UNSUPPORTED` (not a Taiwan account — this is not something code can fix, they need Portaly support) and `PROMOTION_PRICING_TYPE_UNSUPPORTED` (back to step 1).

### 4. Capture the referral code on the creator's site
...
### 5. Attach it when creating the checkout session
...
### 6. Tell the creator what their buyers will see
...
### 7. Verify with a test key, spending nothing

1. Confirm `PORTALY_API_KEY` is `pcs_test_*`.
2. `GET .../promotion` — the switch is on, at the rate you expect.
...
2. The decision the creator has to make (which plan, what rate)
...
10. **Never invent endpoints or fields.** This skill uses exactly the two endpoints above plus `profitSharingId` on checkout-session creation. If something 404s, say the feature isn't enabled on their account and stop — don't smuggle attribution through `metadata` (the Python and Go callback adapters fail closed on custom metadata keys).
11. **Turning promotion on with a live key needs an explicit yes**, with the plan, rate and mode restated first.
...
- the promotion switch result per plan, read back from Portaly
...
- `scripts/check_promotion_setup.mjs` — offline, read-only preflight: prints mode, whether the plan qualifies, and the switch state. Run it before wiring code and again before going live.
