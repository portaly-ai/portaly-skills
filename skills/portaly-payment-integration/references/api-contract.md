...
- Response fields (per plan):
  ...
  - `data[].termsOfService` — the merchant's terms of service for this plan, set in their dashboard. Merchant-authored HTML — sanitize before rendering if you display it yourself. When non-empty, the hosted checkout shows the terms and requires the buyer to tick agreement before paying. Empty or absent when the plan has no terms **or the merchant switched them off** — there is no separate on/off field to read, so treat empty as "no terms step on this checkout". Display-only for you: you send nothing for it, and the callback carries no consent field (a `completed` session implies acceptance).
...
- Request fields:
  ...
  - `metadata`: optional string-keyed extra context. **Echoed into the signed callback body. The Python/Go v1 adapters fail closed on any key outside the committed signing whitelist (see the Signed Callback section) — notably every ad identifier, so keep those in your own store instead.**
  ...
  - `profitSharingId`: optional. The referral code a buyer arrived with when the merchant runs buyer promotion on a plan — the `ps` value from the promotion link, which **your server** reads from its own cookie. Only accepted on this API-key-authenticated create call; a request the buyer's browser can make must never carry it, or anyone could claim someone else's sale. 1–64 characters. **Omit the field entirely when you have no cookie value — an empty string is a validation error (`400`), not a silent ignore.** An unknown, disabled, or mismatched code *is* silently ignored and the checkout still completes. You never create these codes — the merchant's buyers get them from Portaly.
...
- Required headers:
  - `Authorization: Bearer {portaly_payment_api_key}`
- Useful response fields (this endpoint returns the **nested** checkout-session object, not the flat callback payload): `status`, `merchantOrderNumber`, `amount`, `billingPeriod`, `appliedDiscount`, `customer.name`, `customer.email`, `customer.phone`, `collectPhone`, `termsOfService`, `termsOfServiceHtml`, `plan.{id, name, amount, currency, status}`, `expiresAt`, `createdAt`, `updatedAt`
...
- `customerPhone` is present on `checkout.completed` whenever a number was collected: when the plan's `collectPhone` is on (see Read Subscription Plans), and also on a zero-amount live checkout that still saves a card, where the hosted checkout asks for one regardless of the plan setting. It is absent — not empty — otherwise, so treat it as optional and never make it a required column on the way in.
...
- `POST /api/creator-subscription/subscriptions/{subscriptionId}/resume` — body `{}`. Only works before the subscription is fully `canceled`. Sends `creator_subscription.active` when the subscription is `active` (a `past_due` one gets no webhook until a renewal succeeds).
- **`amount` on a subscription is its base price, not what the buyer pays.** It is frozen at checkout (repricing the plan later does not change it) and renewals charge off it. When the subscription carries a `discount` snapshot (`code`, `appliedRule`, `startedAt`, `endsAt` — `null` = forever, `source`, plus `originalAmount` / `finalAmount` on subscriptions created after those were recorded) and that discount is still in effect, the real charge is lower (exception: dynamic-priced plans, always one-time, store the already-discounted session amount in `amount`). Reconcile money against the renewal callback's `amount`, or `GET /orders`, never against `subscriptions[].amount`.
