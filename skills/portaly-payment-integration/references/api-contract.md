...
- Request fields:
  ...
  - `profitSharingId`: optional. The referral code a buyer arrived with when the merchant runs buyer promotion on a plan — the `ps` value from the promotion link, which **your server** reads from its own cookie. Only accepted on this API-key-authenticated create call; a request the buyer's browser can make must never carry it, or anyone could claim someone else's sale. An unknown, disabled, or mismatched code is silently ignored and the checkout still completes. You never create these codes — the merchant's buyers get them from Portaly.
