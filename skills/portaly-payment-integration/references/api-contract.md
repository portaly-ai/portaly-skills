...
- `customerPhone` is present on `checkout.completed` whenever a number was collected: when the plan's `collectPhone` is on (see Read Subscription Plans), and also on a zero-amount live checkout that still saves a card, where the card-binding step requires one regardless of the plan setting. It is absent — not empty — otherwise, so treat it as optional and never make it a required column on the way in.
