...
- `termsOfService` is the plain-text terms copied from the plan when the session was created (empty string when none). A session cannot reach `completed` without the buyer accepting them, so there is no separate consent flag to read.
