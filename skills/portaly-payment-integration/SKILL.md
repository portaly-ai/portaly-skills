...
{ "skillName": "portaly-payment-integration", "version": "0.13.1" }
```
...
- `checkout.completed` is sent only after Portaly has written the subscription and its first order, so your handler may call `GET /subscriptions/{sessionId}` (or cancel/resume) right away. A `404` there right after the callback means the charge was taken but recording failed — contact Portaly support rather than retrying until it appears.
